...
export function PainelReuniao({ eventoId, resumo, presentesOk, valores, observacoes }: { eventoId: string; resumo: string; presentesOk: boolean; valores: DadosReuniaoValores; observacoes: string }) {
  ...
  return (
    <aside className="overflow-hidden rounded-cartao border border-line bg-surface 2xl:sticky 2xl:top-[76px]">
      <TabsControladas
      ...
      {aberto === "dados" ? (
        <>
          <p className="m-0 px-[18px] pt-3 text-pequeno text-muted">{resumo}</p>
