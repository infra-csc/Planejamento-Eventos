...
export function LinhaSolicitacaoEvento({ s }: { s: SolicitacaoLista }) {
  ...
  return (
    <Link href={`/solicitacoes/${s.id}`} className="flex w-full items-center gap-3.5 border-b border-line-row px-[18px] py-[13px] text-left no-underline last:border-b-0 hover:bg-subtle">
      <span className="shrink-0 basis-[78px] font-mono text-pequeno text-ink">{s.codigo}</span>
      <span className="min-w-0 flex-1">
        <span className="block text-corpo text-ink">{s.titulo || "sem título"}</span>
        <span className="block text-pequeno text-muted">
          {s.area.nome} · {s.tipo === "PRE_REUNIAO" ? "pré-reunião" : "alteração"} · por {s.criadoPor.nome}
        </span>
      </span>
      <span className="shrink-0 basis-[96px] font-mono text-pequeno text-ink-3">
      ...
      <span className="shrink-0 basis-[118px] text-right font-mono text-pequeno" style={{ color: COR_TOM[pi.tom] }}>
