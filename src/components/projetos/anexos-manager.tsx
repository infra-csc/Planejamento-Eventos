...
export function AnexosManager({ projetoId, anexos, podeGerenciar }: { projetoId: string; anexos: Anexo[]; podeGerenciar: boolean }) {
  ...
  return (
    <div>
      {anexos.length === 0 && <p className="px-4 py-6 text-center text-sm text-muted">Nenhum anexo. {podeGerenciar ? "Adicione imagens (PNG/JPG/WEBP) ou o PDF técnico." : ""}</p>}
      {imagens.length > 0 && (
        <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-3 md:grid-cols-4">
          {imagens.map((a) => (
            <figure key={a.id} className="group relative">
              ...
              <figcaption className="mt-1 truncate text-xs text-muted">{a.nomeArquivo}</figcaption>
              {podeGerenciar && (
        ...
        <ul className="divide-y divide-line border-t border-line">
          {pdfs.map((a) => (
            <li key={a.id} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
              <a href={`/api/anexos/${a.id}`} target="_blank" className="flex min-w-0 items-center gap-2 text-accent hover:underline">
                <span className="truncate">{a.nomeArquivo}</span> <span className="text-xs text-muted">{tamanho(a.tamanho)}</span>
              </a>
              {podeGerenciar && (
