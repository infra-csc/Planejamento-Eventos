      ...
      <div className="flex flex-col justify-between gap-8 bg-dark px-6 py-6 text-white sm:px-10 lg:px-16 lg:py-14">
        ...
        <div className="flex gap-10 text-corpo text-on-dark-3 max-lg:hidden">
          {n && (
            <>
              <span>
                <span className="block font-mono text-metrica leading-[1.2] text-accent-light">{n.eventos}</span>
                eventos ativos
              </span>
              <span>
                <span className="block font-mono text-metrica leading-[1.2] text-white">{n.pecas}</span>
                peças no catálogo
              </span>
              <span>
                <span className="block font-mono text-metrica leading-[1.2] text-white">{n.projetos}</span>
                projetos padrão
              </span>
            </>
          )}
