...
export default function ErroApp({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div role="alert" className="max-w-[640px] rounded-cartao border border-danger-border bg-danger-bg px-5 py-4">
      <p className="m-0 text-secao font-semibold text-danger">{semConexao ? "Sem conexão com a internet" : "Não foi possível carregar esta página"}</p>
      <p className="mb-0 mt-1 text-corpo leading-[1.5] text-ink-2">
