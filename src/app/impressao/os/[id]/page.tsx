  ...
  return (
    <div className="mx-auto max-w-4xl bg-surface p-8 text-corpo text-ink print:p-0">
      <div className="no-print mb-6 flex items-center justify-between gap-3 print:hidden">
        <Link href={`/eventos/${id}/os`} className="link text-corpo print:hidden">
          ← Voltar para a OS
        </Link>
        <ImprimirBotao />
      </div>
      <header className="mb-6 border-b border-ink pb-4">
        <p className="text-rotulo uppercase tracking-widest">Norte Mkt · Ordem de Serviço</p>
        <h1 className="mt-1 text-pagina font-semibold">
          {ev.codigo} · {ev.nome}
        </h1>
        <div className="mt-2 grid grid-cols-2 gap-x-6 gap-y-1 sm:grid-cols-4">
          <div>
            <span className="text-ink-3">Evento:</span> {formatarPeriodo(ev.dataInicio, ev.dataFim)}
          </div>
          <div>
            <span className="text-ink-3">Reunião de OS:</span> {formatarDataHora(ev.dataReuniao)}
          </div>
          <div>
            <span className="text-ink-3">Local:</span> {ev.local || "—"}
          </div>
          <div>
            <span className="text-ink-3">Versão:</span> {v && sel ? `v${sel.numero} (${formatarDataHora(sel.geradaEm)})` : versoes[0] ? `v${versoes[0].numero} · atual` : "prévia"}
          </div>
        </div>
      </header>

      <h2 className="mb-3 text-titulo font-semibold">Totais por peça</h2>
      {os.setores.map((s) => (
        <section key={s.setor} className="mb-8 break-inside-avoid">
          <h2 className="mb-2 text-destaque font-semibold">{SETOR_LABEL[s.setor]}</h2>
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-ink text-left text-rotulo uppercase tracking-wide">
                ...
                <th scope="col" className="py-1 pr-3">Peça</th>
                <th scope="col" className="py-1 pr-3 text-right">Total</th>
                <th scope="col" className="py-1 pr-3">Un.</th>
                <th scope="col" className="py-1">Composição</th>
                <th scope="col" className="py-1 w-16 text-center">Sep.</th>
              </tr>
            </thead>
            <tbody>
              {s.linhas.map((l) => (
                <tr key={l.pecaId} className="border-b border-line-strong">
                  ...
                  <td className="py-1.5 text-rotulo text-ink-3">{l.origens.map((o) => `${o.descricao} → ${o.quantidade}`).join(" · ")}</td>
      ...
      {os.projetos && os.projetos.length > 0 && (
        <section className="mb-8 break-before-page">
          <h2 className="mb-1 text-titulo font-semibold">Por projeto</h2>
          <p className="mb-4 text-rotulo text-ink-3">O que cada projeto padrão leva, já com os ajustes pedidos pela área. As mesmas peças estão somadas nos totais acima.</p>
          {os.projetos.map((p, i) => (
            <div key={`${p.codigo}-${i}`} className="mb-6 break-inside-avoid">
              <h3 className="mb-0.5 text-secao font-semibold">
                {p.nome} <span className="font-mono text-rotulo font-normal text-ink-3">{p.codigo} · v{p.versao}</span> <span className="ml-1 rounded-chip bg-ink px-1.5 py-px font-mono text-rotulo text-white">× {p.quantidade}</span>
              </h3>
              <p className="mb-2 text-rotulo text-ink-3">{[p.destino ? `Destino: ${p.destino}` : null, p.area, `${p.pecas.length} tipos de peça · ${p.pecas.reduce((a, x) => a + x.total, 0)} unidades`].filter(Boolean).join(" · ")}</p>
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-ink text-left text-rotulo uppercase tracking-wide">
                    <th scope="col" className="py-1 pr-3">Código</th>
                    <th scope="col" className="py-1 pr-3">Peça</th>
                    <th scope="col" className="py-1 pr-3">Setor</th>
                    <th scope="col" className="py-1 pr-3 text-right">Por un.</th>
                    <th scope="col" className="py-1 pr-3 text-right">Total</th>
                    <th scope="col" className="py-1 pr-3">Un.</th>
                    <th scope="col" className="py-1 w-16 text-center">Sep.</th>
                  </tr>
                ...
                <tbody>
                  {p.pecas.map((x) => (
                    <tr key={x.codigo} className="border-b border-line-strong">
                      <td className="py-1.5 pr-3 font-medium">{x.codigo}</td>
                      <td className="py-1.5 pr-3">{x.nome}</td>
                      <td className="py-1.5 pr-3 text-rotulo text-ink-3">{SETOR_LABEL[x.setor]}</td>
      ...
      {os.individuais && os.individuais.length > 0 && (
        <section className="mb-8 break-inside-avoid">
          <h2 className="mb-1 text-destaque font-semibold">Peças pedidas soltas</h2>
          <p className="mb-2 text-rotulo text-ink-3">Fora de projeto padrão. Também somadas nos totais.</p>
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-ink text-left text-rotulo uppercase tracking-wide">
                <th scope="col" className="py-1 pr-3">Código</th>
                ...
                <th scope="col" className="py-1 pr-3">Un.</th>
                <th scope="col" className="py-1 pr-3">Destino · área</th>
                <th scope="col" className="py-1 w-16 text-center">Sep.</th>
              </tr>
            </thead>
            <tbody>
              {os.individuais.map((x, i) => (
                <tr key={`${x.codigo}-${i}`} className="border-b border-line-strong">
                  <td className="py-1.5 pr-3 font-medium">{x.codigo}</td>
                  <td className="py-1.5 pr-3">{x.nome}</td>
                  <td className="py-1.5 pr-3 text-rotulo text-ink-3">{SETOR_LABEL[x.setor]}</td>
                  <td className="py-1.5 pr-3 text-right font-mono font-semibold">{x.quantidade}</td>
                  <td className="py-1.5 pr-3">{x.unidade}</td>
                  <td className="py-1.5 pr-3 text-rotulo text-ink-3">{[x.destino, x.area].filter(Boolean).join(" · ") || "—"}</td>
                  <td className="py-1.5 text-center">☐</td>
                </tr>
              ))}
            </tbody>
      ...
      {os.semSetor.length > 0 && (
        <section className="mb-8 break-inside-avoid">
          <h2 className="mb-2 text-destaque font-semibold">Itens avulsos</h2>
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-ink text-left text-rotulo uppercase tracking-wide">
                <th scope="col" className="py-1 pr-3">Descrição</th>
                <th scope="col" className="py-1 pr-3 text-right">Qtd.</th>
                <th scope="col" className="py-1 pr-3">Destino</th>
                <th scope="col" className="py-1">Área</th>
                <th scope="col" className="py-1 w-16 text-center">Sep.</th>
              </tr>
            </thead>
            <tbody>
