 ...
 * Sessões abertas desse usuário são encerradas.
 */
...
async function main() {
  ...
  if (u) {
    await db.update(usuarios).set({ senhaHash, ativo: true, perfil: "ADMIN", areaId: null }).where(eq(usuarios.id, u.id));
    ...
    await db.insert(usuarios).values({ nome: "Administrador do Sistema", email: email.trim(), senhaHash, perfil: "ADMIN", ativo: true });
